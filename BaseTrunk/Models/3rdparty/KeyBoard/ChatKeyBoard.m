...
{
    return [self keyBoardWithNavgationBarTranslucent:NO];
}
...
+ (instancetype)keyBoardWithNavgationBarTranslucent:(BOOL)translucent
{
    ...
    if (translucent) {
        frame = CGRectMake(0, kScreenHeight - Tabbar_Height, kScreenWidth, kChatKeyBoardHeight);
    } else {
        frame = CGRectMake(0, kScreenHeight - Tabbar_Height - Navbar_Height, kScreenWidth, kChatKeyBoardHeight);
    }
...
- (void)keyBoardWillChangeFrame:(NSNotification *)notification
{
    ...
    else
    {
        [UIView animateWithDuration:0.25 animations:^{
            ...
            {
                CGFloat chatToolBarHeight = kChatToolBarAnimalHeight;
            ...
            {
                CGFloat chatToolBarHeight = kChatToolBarHeight;
                CGFloat targetY = end.origin.y - chatToolBarHeight - (kScreenHeight - [self getSuperViewH]);
                self.lastChatKeyboardY = self.frame.origin.y;
                //键盘收起
...
{
    if (!_facePanel) {
        _facePanel = [[FacePanel alloc] initWithFrame:CGRectMake(0, kChatToolBarHeight, kScreenWidth, kFacePanelHeight)];
