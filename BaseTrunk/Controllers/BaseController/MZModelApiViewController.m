...
@interface MZModelApiViewController()
@property (nonatomic, strong) id model;
@end


@implementation MZModelApiViewController
@synthesize model;
...
- (void)loadData:(NSURLRequestCachePolicy)cachePolicy more:(BOOL)more
{
    ...
    MZParamsBaseObject *params = [self paramsObject:_loadMore];
    if (params && [params isKindOfClass:[MZParamsBaseObject class]]) {
        [[MZFileClient sharedInstance] request_send:params cachePolicy:cachePolicy delegate:self selector:@selector(requestDidFinishLoad:) selectorError:@selector(requestDidError:)];
    }
}
