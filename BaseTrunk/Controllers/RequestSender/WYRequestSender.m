...
+ (instancetype)currentClient
{
    ...
    dispatch_once(&predicate, ^{
        sharedInstance = [[WYRequestSender alloc] init];
    });
...
{
    if(!OBJ_IS_NIL([WYUserInfoObject sharedInstance].token))
    {
        [request addValue:[WYUserInfoObject sharedInstance].token forHTTPHeaderField:@"token"];
    }
