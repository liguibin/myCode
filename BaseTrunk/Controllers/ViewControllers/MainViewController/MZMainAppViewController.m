...
#import "MZMainAppViewController.h"
#import "MZUserInfoObject.h"
#import "MZItemParseBase.h"
#import "WYSocketObject.h"

@interface MZMainAppViewController()<WYSocketObjectDelegate>
@end

@implementation MZMainAppViewController
...
- (void)viewDidLoad
{
    ...
    self.viewControllers = [NSArray arrayWithObjects:
                            [self viewControllerWithTabTitle:(@"首页") image:nil finishedSelectedImage:nil viewClass:@"MZHomeViewController"],
                            [self viewControllerWithTabTitle:@"消息" image:nil finishedSelectedImage:nil viewClass:@"MZChatListViewController"],
                            [self viewControllerWithTabTitle:(@"我的") image:nil finishedSelectedImage:nil viewClass:@"MZProfileViewController"],
                            nil];
...
{
    [MZUserInfoObject saveUserInfo:[MZUserInfoObject sharedInstance]];
}
...
{
    //1.判断是否登录
    if([[MZUserInfoObject sharedInstance] getToken] && ![[[MZUserInfoObject sharedInstance] getToken] isEqualToString:@""])
    {
...
- (void)appUpdate:(NSDictionary *)appUpdateInfo
{
    if (appUpdateInfo && [appUpdateInfo isKindOfClass:[NSDictionary class]]) {
        BOOL update = [MZItemParseBase getBoolValue:[appUpdateInfo objectForKey:@"update"]];
        if (update) {
            //need update
            NSString *update_version = [MZItemParseBase getStrValue:[appUpdateInfo objectForKey:@"version"]];
...
- (void)socketReceiveData:(NSDictionary *)data
{
    if (data && [data isKindOfClass:[NSDictionary class]]) {
        ...
        if (payload && [payload isKindOfClass:[NSDictionary class]]) {
            ...
            NSString *type = [MZItemParseBase getStrValue:[payload objectForKey:@"type"]];
